...
public enum ModelNames {
    ...
    static func getRequiredModelNames(for repo: Repo, variant: String?) -> Set<String> {
        ...
        case .pocketTts:
            // TTS catalog not vendored (Power Tools' ASR-only usage never
            // requests this repo) — see NOTICE.md.
            return []
        case .kokoroAne:
        ...
        case .sortformer:
            ...
            // Diarizer catalog not vendored (Power Tools' ASR-only usage
            // never requests this repo) — see NOTICE.md.
            return []
        case .lseendAmi, .lseendCallHome, .lseendDihard2, .lseendDihard3:
            if let variant = variant {
