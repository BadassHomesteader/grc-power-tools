...
- **Notch strip** — one live glance in the camera housing, shared by several features rather than owned by any one of them. Collapsed it is a row of dots flanking the notch: one per Claude Code / Codex / Cursor session, tinted by state, plus a quota dot once a usage window runs low. Hover a dot and the notch opens into a small card; when a session asks for permission the card appears on its own with **✓ / ✕** on it, so you answer without opening anything. Click a dot to pin it open; **click anywhere else** — another app, another window — and it folds back into the housing (the ✕ on the tab row does the same). Hover the **four-square mark** at the end of the strip and a row of small panels opens (Usage, Hotkeys, Snap, Clock, Weather, Ask, and a ⚙ Settings tile that opens the app's settings window); move away and it folds back, click one to keep it, and a tab row moves between them or out. When it opens it grows out of the camera housing — rounded outer corners and a concave curve on each side of the notch, so the panel reads as part of the hardware rather than a slab hung under it. It never grows past a small panel below the housing — the notch is a glance, not a window — and it draws **nothing at all** when there is nothing to say. Everything is optional in Settings ▸ Notch.
