...
Your hotkey doubles as a *leader* — hold **Option + Shift** and the overlay shows the menu (`A ai · T text · R read · S shot · G lens · K color` / `C X V files · Y shelf · W palette · ← → ↑ ↓ win`). Keep holding and tap a key, or just speak. Until you speak, the overlay shows the hint strip; the moment you talk it swaps to the live waveform.
...
- **Notch strip** — one live glance in the camera housing, shared by several features rather than owned by any one of them. Collapsed it is a row of dots flanking the notch: one per Claude Code / Codex / Cursor session, tinted by state, plus a quota dot once a usage window runs low. Hover a dot and the housing opens into the **agent list** — one row per session with its app's icon, state, model and program chips, branch, task, what it is doing right now, how long it has run and what it has spent; a waiting permission carries **✓ / ✕** on its row, so you answer without opening anything, and the last few finished sessions sit dimmed under **Recent**, each with an ✕ to clear it. The band beside the camera says how many are active and carries ↻ and the module launcher. The module row stays with the list, so you can go straight from a session to Weather or Disk without folding it away. Click a dot to pin it open; **click anywhere else** — another app, another window — and it folds back into the housing (the ✕ in the band beside the camera does the same). Hover the **four-square mark** at the end of the strip and a single row of module icons opens — the same row, same size, that a module’s tab bar shows, so nothing moves under the cursor when one opens (Usage, Hotkeys, Snap, Clock, a Calendar month with today ringed, a MacNotch-style Weather panel with feels-like, humidity, wind, UV, rain chance, pressure, sunrise/sunset and the next hours for each of your cities, a **System** panel — CPU, GPU and memory gauges over load, processes, uptime, memory pressure, swap, and the battery’s charge, health, wattage and temperature — a **Camera** mirror to check yourself before a call — it opens on a click, never a hover, and the camera runs only while the panel is open — a **Disk** panel — each volume’s used/free bar with an ⏏ for anything ejectable, over the live read/write throughput and since-boot totals of each physical device — Ask, an Agent Pad tile that shows the agent list in the notch on hover, a Macro Pad tile that toggles the pad, and a ⚙ Settings tile that opens the app's settings window); move away and it folds back, click one to keep it, and a tab row moves between them or out. When it opens it grows out of the camera housing — the outer top corners round off and a large concave cove sweeps up into each side of the notch, so the panel reads as flowing out of the housing rather than a slab hung under it. Modules stay a small panel below the housing — the notch is a glance, not a window; the agent list is the one shape allowed to go wide — and with no agents running it keeps just the **four-square launcher** beside the camera, so the panels stay one hover away. Everything is optional in Settings ▸ Notch.

- **hold + Y** → **Shelf**: a tray you drag files, text and images onto to park them, then drag back out somewhere else — the drag nobody can do in one go, across Spaces, between two windows that can’t both be on screen, or out of a mail message into a folder you haven’t opened yet. Drop several files at once; re-dropping one moves it back to the top rather than doubling it. The header carries a **Copy / Move** switch. On **Copy** (the default) dragging an item out never moves the original and the row stays, so you can drop the same file in five places; on **Move** the file is really relocated to wherever you drop it and the row goes with it. Text and images always copy — they have nowhere to move from. The mode is always visible in the header rather than hidden behind a modifier held at drag time, and it survives a restart. Rows go away with the ✕ per row, or **Clear**. Double-click opens a file or copies text/an image; right-click gives Copy · Reveal in Finder · Remove. Drag it by its header to dock it against any edge or corner, where it stays across restarts. The shelf itself is **session-only**: quitting empties it, and only its position is remembered. Toggle also in the menu bar.
